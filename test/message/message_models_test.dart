...
void main() {
  group('ConversationModel.fromMap', () {
    test(
      'parses all fields with joined last message and other participant',
      () {
        final map = {
          'id': 'conv1',
    ...
    test('uses display name when the other participant has no username', () {
      ...
      expect(model.otherUsername, 'Maria Santos');
    });
  });
  ...
  group('MessageModel.fromMap', () {
    ...
    test('uses display name when the sender has no username', () {
      ...
      expect(model.senderUsername, 'Juan Dela Cruz');
    });
